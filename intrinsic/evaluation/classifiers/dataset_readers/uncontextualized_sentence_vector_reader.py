...
@DatasetReader.register("uncontextualized_sentence_vectors")
class WordVectorDatasetReader(DatasetReader):
	"""
	Reads a text file for classification task. Reads only the token for which a label is present.
	...
	def text_to_instance(self, tokens: List[Token], label: str = None) -> Instance:
		...
		word_index = tokens[-1]
		token = tokens[int(word_index.text)]
		fields = {'token': TextField([token], self._token_indexers)}
